        ...
        for index, row in book_df.iterrows():
            ...
            for title_indiv_word in formatter(row["bookTitle"].lower()):
                if word.lower() in title_indiv_word:
                    match_ids.append(row["bookID"])

    #Get corresponding records from MongoDB
    match_dict = {}
    for b in mongo_output(connect.collection.find({"_id": {"$in": match_ids}})):
...
def adv_helper(match_dict, b):
    ...
    for index, row in book_df.iterrows():
        if row["bookID"] == b["_id"]:
            data_packet = [row["bookTitle"]]
            
            if row["borrowMemberID"] == None:
                if row["reserveMemberID"] == None:
                    # Book not borrowed and not reserved
                    data_packet.extend(["Available", "Not Reserved", "NIL"])
                else:
                    # Book not borrowed but is reserved
                    data_packet.extend(["Not Available", "Reserved", "NIL"])
            else:
                if row["reserveMemberID"] == None:
                    # Book borrowed and not reserved
                    data_packet.extend(["On Loan", "Not Reserved", row["dateDue"]])
                else:
                    # Book borrowed but is reserved
                    data_packet.extend(["On Loan", "Reserved", row["dateDue"]])
            
            #Add misc 
            data_packet.append(",".join(b["authors"][:2]))\
                
            desc = ""
            if "shortDescription" in b:
                desc = b["shortDescription"]
            elif "longDescription" in b:
                sentences = b["longDescription"].split(".")
                desc = ".".join(sentences[:2])
            else:
                desc = "No Description Available"
            data_packet.append(desc)
            
            if "thumbnailUrl" in b:
                data_packet.append(b["thumbnailUrl"])
            else:
                data_packet.append("#")
            
            match_dict[row["bookID"]] = data_packet
    return match_dict
